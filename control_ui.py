...
class ADSApp:
    ...
    def build_ui(self):
        ...
        self.pos_tree = ttk.Treeview(pos_frame, columns=pos_columns, show="headings", height=8)
    ...
    def update_mover_positions(self, all_values):
        try:
            ...
            for item in self.pos_tree.get_children():
                current_values = self.pos_tree.item(item, "values")
                mover_id = current_values[0] if current_values else "?"
                self.pos_tree.item(item, values=(mover_id, "ERR", "ERR", "ERR"))
            return
        ...
        if mover_count > existing_rows:
            ...
            for i in range(existing_rows, mover_count, -1):
                row_id = f"mover_{i}"
                if self.pos_tree.exists(row_id):
                    self.pos_tree.delete(row_id)
        ...
        for i in range(1, mover_count + 1):
            ...
            try:
                ...
                self.pos_tree.item(row_id, values=(i, f"{x:.3f}", f"{y:.3f}", f"{z:.3f}"))
                all_values[f"Mover{i}_X"] = x
                all_values[f"Mover{i}_Y"] = y
                all_values[f"Mover{i}_Z"] = z
            except Exception as e:
                self.pos_tree.item(row_id, values=(i, "ERR", "ERR", "ERR"))
    ...
    def poll_loop(self):
        while self.polling:
            ...
            try:
                delay_s = max(float(self.poll_ms.get()) / 1000.0, 0.05)
            except ValueError:
                delay_s = 0.3
